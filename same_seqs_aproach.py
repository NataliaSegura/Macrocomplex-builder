...
def has_homolgs(seq, seqs):
    for s in seqs:
        total_len = 0
        ident = 0
        alignment = align.globalxx(seq, s)
        for i in range(len(alignment[0][0])):
            if alignment[0][0][i] == alignment[0][1][i]:
                ident += 1
            total_len += 1
        if ident/total_len >= 0.95:
            return s
    ...
    for pdb in pdbmodels:
        ...
        if chain1_seq not in seq_dict:
            if not seq_dict:
                ...
                homolog = has_homolgs(chain1_seq, sequences)
                if homolog:
                    seq_dict[chain1_seq] = seq_dict[homolog]
                    chain1.id = seq_dict[homolog]
                else:
                    new_id = get_new_id(seq_dict.values())
                    seq_dict[chain1_seq] = new_id
                    chain1.id = new_id
        ...
        if chain2_seq not in seq_dict:
            sequences = seq_dict.keys()
            homolog = has_homolgs(chain2_seq, sequences)
            if homolog:
                seq_dict[chain2_seq] = seq_dict[homolog]
                chain2.id = seq_dict[homolog]
            else:
                new_id = get_new_id(seq_dict.values())
...
def get_sequence(chain):
    ...
    seq = ""
    flag = "prot"
    first_residue_name = chain.child_list[0].resname.strip()
...
def get_common_atoms(chain,fixed):
        ...
        if counter < 200:
            if len(chain.interactions) > 0:
                for inter_tple in chain.interactions:
                    ...
                    chain_atoms, fix_atoms = get_common_atoms(chain, fix)
